...
void main() {
    ...
    if (isFC > 0) {
		finalColor = (texture(tex2, middlePush) * 2) + (texture(tex1, topPush) * 2) + (texture(baseTex, basePush) / 4);
        //finalColor = clamp(
		//	(
		//		(
		//			(texture2D(tex2, middlePush)) * 2
		//		) + 
		//		(
		//			(texture2D(tex1, topPush)) * 2
		//		) + 
		//		(
		//			(texture2D(baseTex, basePush)) / 4
		//		)
		//	) * 5.0f, 
		//	FCColor * 1.5f, 
