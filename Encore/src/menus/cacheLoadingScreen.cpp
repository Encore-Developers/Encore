...
void cacheLoadingScreen::Load() {
	...
	JosefinSansItalic = GameMenu::LoadFontFilter(assetsdir / "fonts/JosefinSans-Italic.ttf");
...
void cacheLoadingScreen::Draw() {
	...
	GameMenu::mhDrawText(RedHatDisplay, "LOADING CACHE",
	...
	GameMenu::mhDrawText(RubikBold, LoadingText, {u.RightSide - lwidth, u.hpct(0.085f)}, RubikFontSize, LIGHTGRAY);
	...
	GameMenu::mhDrawText(JosefinSansItalic, CacheSplash[SplashSel], {u.LeftSide + u.hinpct(0.16), GetScreenHeight() - u.hpct(0.14f) + u.hinpct(0.055f)},  RubikFontSize/1.5f, WHITE);
	if (!started) {
		started = true;
