...
LerpState Lerp::createLerp(
    std::string key, easing_functions ease, float duration, bool startAutomatically
) {
...
void Lerp::updateStates() {
    ...
    for (auto &it : this->activeLerps) {
        ...
        it.second.value = clamp(
            getEasingFunction(it.second.func)(it.second.elapsed / it.second.totalDuration),
            1,
            0
        );
        printf(
            "State %s, elapsed %f/%f, actual eased value: %f\n",
            it.first.c_str(),
            it.second.elapsed,
